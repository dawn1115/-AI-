...
(function (vue, pinia, rxjs, md5, ElementPlus) {
  ...
  const _hoisted_3$2 = /* @__PURE__ */ vue.createElementVNode("p", null, '2脚本没有开发自己到题库，而是接入了AI,获取密钥+v:lingf1115', -1);
  ...
  const _hoisted_6$1 = /* @__PURE__ */ vue.createElementVNode("p", null, "-- 题库密钥请确认能够搜索到题目再获取，题库均为AI作答，出现任何问题与脚本无关。如果你是程序员，可以自行接入自己的题库，这里不提供任何教程，也不会回复任何询问，请自行查看源代码修改即可，不会改的绕道。", -1);
  const _hoisted_7$1 = [
    _hoisted_2$2,
    _hoisted_3$2,
    _hoisted_4$1,
  ...
  const getAnswerFrom = async (question, url2) => {
    ...
    const buildPrompt = (q) => {
      ...
      const workType = (q.workType || '').trim();
      const optionsText = Array.isArray(q.optionsText) ? q.optionsText.join('\n') : (q.optionsText || '');
      return [
        `请根据题目给出直接可用的标准答案。`,
        `要求:`,
        `- 若是单选题，返回选项文本。`,
        `- 若是多选题，返回选项文本，多个选项用/隔开。`,
        `- 若是判断题，返回“正确”或“错误”。`,
        `- 若是填空题，返回填空内容。`,
        `- 避免解释，直接给答案。`,
        `题目类型: ${type} | 作业类型: ${workType}`,
        `题目: ${title}`,
        optionsText ? `选项:\n${optionsText}` : ``,
      ].filter(Boolean).join('\n');
    };

    const normalizeAiAnswer = (q, content) => {
      const text = (content || '').trim();
      if (!text) return [];
      
      console.log('AI原始回复:', text); // 调试日志
      
      // 首先检查是否是选择题（单选或多选）
      if (q.type === '单选题' || q.type === '多选题' || q.type === '0' || q.type === '1') {
        // 格式1: 多选题 - 选项文本用/分隔
        if (q.type === '多选题' || q.type === '1') {
          // 检查是否包含/分隔的选项文本
          const slashSeparatedMatch = text.match(/答案[:：]\s*(.+)/i);
          if (slashSeparatedMatch && slashSeparatedMatch[1]) {
            const options = slashSeparatedMatch[1].split('/').map(opt => opt.trim()).filter(opt => opt);
            if (options.length > 0) {
              console.log('从格式1提取到的多选题选项文本:', options);
              return options;
            }
          }
        }
        ...
        // 格式2: 单选题 - 直接提取选项文本
        const optionTextMatch = text.match(/答案[:：]\s*(.+)/i);
        if (optionTextMatch && optionTextMatch[1]) {
          const optionText = optionTextMatch[1].trim();
          console.log('从格式2提取到的选项文本:', [optionText]);
          return [optionText];
        }
        
        // 向后兼容：格式3 - 选项字母（A-Z）
        const letterMatches = text.match(/[A-Za-z]/gi);
        if (letterMatches && letterMatches.length > 0) {
          const uniqueLetters = [...new Set(letterMatches.map(l => l.toUpperCase()))];
          console.log('从格式3提取到的字母(兼容模式):', uniqueLetters);
      ...
      // 其它情况直接返回全文（填空、简答）
      console.log('返回原文:', [text]); // 调试日志
      return [text];
    };

    const makeRequestBody = (prompt) => JSON.stringify({
      model: 'x1',
      user: _self.uid || _self.getCookie('UID') || _self.getCookie('_uid') || 'user_id',
      messages: [{ role: 'user', content: prompt }],
      stream: false
    });

    // 读取用户填入的API密钥
    const sparkKeyCandidate = (configStore.queryApis?.[0]?.token || '').trim();
    const authHeader = sparkKeyCandidate
      ? (sparkKeyCandidate.startsWith('Bearer ') ? sparkKeyCandidate : `Bearer ${sparkKeyCandidate}`)
      : '';
    const headers = {
    ...
    // 发送前校验密钥，避免401无意义重试
    if (!authHeader) {
      logStore.addLog('星火API密钥未配置：请在设置或控制台配置', 'danger');
      return handleError('未配置星火 API 密钥');
    }
    const rawToken = authHeader.replace(/^Bearer\s+/, '');
    if (!rawToken.includes(':')) {
      logStore.addLog('星火API密钥格式错误：需 Bearer key:secret', 'danger');
      return handleError('认证格式错误');
    }

    const attempt = (retryIndex) => new Promise((resolve) => {
      const scriptInfo = getScriptInfo();
      const prompt = buildPrompt(question);
      const body = makeRequestBody(prompt);

      // 若 GM_xmlhttpRequest 不可用，使用 fetch + AbortController 回退
      if (!_GM_xmlhttpRequest) {
        const controller = new AbortController();
        const timer = setTimeout(() => controller.abort(), timeoutMs);
        fetch(url2 + `?s=${scriptInfo.author}&v=${scriptInfo.version}`, {
          method: 'POST',
          headers,
          body,
          ...
          try {
            ...
            console.log('星火API原始响应(fetch):', text);
            const raw = JSON.parse(text);
            console.log('解析后的响应(fetch):', raw);
            
            if (resp.status >= 400) {
              const msg = resp.status === 401 ? '认证失败（Authorization）' : `HTTP错误 ${resp.status}`;
              console.warn('HTTP错误(fetch):', resp.status, raw?.message);
              resolve(handleError(raw?.message || msg));
              return;
            }
            
            if (raw && typeof raw.code !== 'undefined' && raw.code !== 0) {
              console.log('API返回错误码(fetch):', raw.code, raw.message);
              resolve(handleError(`API错误: ${raw.message || '未知错误'}`));
              return;
            }
            
            const content = raw?.choices?.[0]?.message?.content || '';
            console.log('提取的content(fetch):', content);
            const answer = normalizeAiAnswer(question, content);
            console.log('规范化后的答案(fetch):', answer);
            if (!answer || answer.length === 0) {
              resolve(handleError(raw?.message || '未提取到答案'));
              return;
            }
            resolve({
              code: 200,
      ...
      _GM_xmlhttpRequest({
        ...
        onload: (response) => {
          try {
            console.log('星火API原始响应:', response.responseText); // 调试日志
            const raw = JSON.parse(response.responseText);
            console.log('解析后的响应:', raw); // 调试日志
            
            // HTTP状态码检查
            if (response.status >= 400) {
              const msg = response.status === 401 ? '认证失败（Authorization）' : `HTTP错误 ${response.status}`;
              console.warn('HTTP错误:', response.status, raw?.message); // 调试日志
              resolve(handleError(raw?.message || msg));
              return;
            }
            
            // 星火接口通常成功为 code === 0，但部分错误没有code字段，仅message
            if (raw && typeof raw.code !== 'undefined' && raw.code !== 0) {
              console.log('API返回错误码:', raw.code, raw.message); // 调试日志
              resolve(handleError(`API错误: ${raw.message || '未知错误'}`));
              return;
            }
            
            const content = raw?.choices?.[0]?.message?.content || '';
            console.log('提取的content:', content); // 调试日志
            
            const answer = normalizeAiAnswer(question, content);
            console.log('规范化后的答案:', answer); // 调试日志
            
            // 若未能提取到任何答案，视为失败，避免后续fillQuestion崩溃
            if (!answer || answer.length === 0) {
              resolve(handleError(raw?.message || '未提取到答案'));
              return;
            }
    ...
    for (let i = 0; i <= maxRetries; i++) {
      ...
      lastResult = res;
      if (res.code !== 10003) break; // 成功或确定错误
    }
    return lastResult;
  };
      ...
      __publicField(this, "init", async () => {
        ...
        if (this.questions.length) {
          ...
          for (const [index, question] of this.questions.entries()) {
            ...
            const timeoutResult = handleError('AI接口超时');
            this.addLog(`第${index + 1}题开始请求AI接口...`, "warning");
            const answerData = await Promise.race([
              getAnswer(question),
              new Promise((resolve) => setTimeout(() => resolve(timeoutResult), TIMEOUT_MS))
            ]);
      ...
      __publicField(this, "fillQuestion", (question) => {
        ...
        if (question.type === "0" || question.type === "1") {
          question.answer.forEach((answer) => {
            ...
            const cleanAnswerUpper = cleanAnswer.toUpperCase();
            
            for (const key in question.options) {
              const cleanKey = this.removeHtml(key).trim();
              let matchFound = false;
              
              // 尝试三种匹配方式：
              // 1. 文本完全匹配（提示词要求的选项文本匹配）
              if (cleanKey.includes(cleanAnswer) || cleanAnswer.includes(cleanKey)) {
                console.log('文本匹配成功:', cleanAnswer, '在', cleanKey);
                matchFound = true;
              }
              // 2. 直接字母匹配（兼容字母形式的答案）
              else if (cleanKey === cleanAnswerUpper) {
                console.log('字母直接匹配成功:', cleanAnswerUpper);
                matchFound = true;
              }
              // 3. 提取选项文本开头的字母进行匹配
              else {
                const optionLetter = cleanKey.match(/^([A-Za-z])/);
                if (optionLetter && optionLetter[1].toUpperCase() === cleanAnswerUpper) {
                  console.log('字母前缀匹配成功:', optionLetter[1].toUpperCase());
                  matchFound = true;
                }
              }
